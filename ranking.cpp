...
void DrawRanking(void)
{
	...
	// 背景の描画
	InitDraw(pDevice, s_pVtxBuffRankbg);	// Drawの初期化
	SetDraw(pDevice, s_pTextureRankbg, 0);	// 描画処理
	...
	// スコアの描画
	InitDraw(pDevice, s_pVtxBuffRankScore);	// Drawの初期化
	for (nCntRank = 0; nCntRank < MAX_RANK; nCntRank++)
	{// 順位
		for (nCntScore = 0; nCntScore < MAX_RANKSCORE; nCntScore++)
		{// スコア
			SetDraw(pDevice, s_pTextureRankScore, (nCntRank * 4 * MAX_RANKSCORE) + (nCntScore * 4));	// 描画処理
		}
	}
}
