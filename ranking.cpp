...
void DrawRanking(void)
{
	...
	RectDraw(pDevice, s_pTextureBg, 0);	// 描画処理

	// 背景の描画
	InitDraw(pDevice, s_pVtxBuffMenu);	// Drawの初期化
	RectDraw(pDevice, s_pTextureMenu, 0);	// 描画処理

	// 順位の描画
	InitDraw(pDevice, s_pVtxBuffRank);		// Drawの初期化
	for (nCntRank = 0; nCntRank < MAX_RANK; nCntRank++)
	{
		RectDraw(pDevice, s_pTextureRank, nCntRank * 4);	// 描画処理
	}
	...
	{// 順位
		for (nCntScore = 0; nCntScore < MAX_SCORE; nCntScore++)
		{// スコア
			RectDraw(pDevice, s_pTextureScore, (nCntRank * 4 * MAX_SCORE) + (nCntScore * 4));	// 描画処理
		}
	}
	...
	for (int i = 0; i < SELECT_NUMBER; i++)
	{
		RectDraw(pDevice, s_pTextureSelect[i], i * 4);	// 描画処理
	}
}
